    ...
    for attempt in range(retries + 1):
        try:
            ...
            if not idempotent or attempt == retries:
                raise
            _log.warning("Plaky %s %s transient network error (attempt %d): %s", method, url, attempt + 1, exc)
        ...
        if response.status_code in _TRANSIENT_STATUSES and idempotent and attempt < retries:
            _log.warning("Plaky %s %s transient %d (attempt %d); retrying", method, url, response.status_code, attempt + 1)
            await asyncio.sleep(_transient_backoff(attempt))
            continue

...
async def _live_repo_keys() -> list[str] | None:
    """Live GitHub org repo names for repo-mention matching — never a hardcoded list.

    Backed by ``list_workspace_repos``'s own TTL cache (org_repos.py), so this is a
    cheap in-process lookup on every turn after the first, not a GitHub call per message.
    Returns None (falls back to the yaml-only registry) if the fetch fails so a transient
    GitHub outage degrades repo scoping instead of breaking the whole turn.
    """
    from boardman.repos_config import list_workspace_repos

    try:
        return list(await list_workspace_repos())
    except Exception:
        logger.warning(
            "Live org repo fetch failed; falling back to configured repos", exc_info=True
        )
        return None
...
async def run_agent_chat(
    ...
    *,
    ...
    resolution = resolve_repo(
        ...
        message=message,
        known_repo_keys=await _live_repo_keys(),
    )

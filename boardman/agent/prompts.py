...
**Missing docs are NOT a blocker.** `DIRECTION.md`/README are preferred but optional. `github_repo_planning_context` always returns a `structure_overview` (description, topics, language mix, top-level dirs, entry points, manifests) and `manifest_excerpts` even when a repo has zero markdown files — explain the repo from those, then read entry-point/source files via **github_fetch_file** for depth. Never answer "I need DIRECTION.md/README to proceed"; instead dive in, state which signals you used (`context_sources`), note that docs are missing, and (optionally) suggest seeding one via `boardman init owner/repo`. The same applies locally: **scan_local_repo** returns `structure_summary` + `manifests` when no docs exist.

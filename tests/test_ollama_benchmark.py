...
class OllamaBenchmark:
    ...
    async def run_latency_test(
        ...
        return BenchmarkMetrics(
            ...
            ttft_sec=avg_ttft,
            tokens_per_sec=(
                actual_completion / avg_time if avg_time > 0 and actual_completion > 0 else 0
            ),
    ...
    async def run_concurrent_test(
        ...
        total_tokens = sum(
            estimate_tokens(r[0].get("message", {}).get("content", "")) for r in successful
        )
    ...
    async def run_cache_test(
        ...
        for _ in range(num_repeats):
            _, t, _ = await chat_complete_async(self.model, messages, timeout=60.0, stream=False)
            times.append(t)
...
class TestOllamaComprehensiveBenchmark:
    ...
    async def test_baseline_latency_short_prompt(self, model):
        ...
        print(f"\n{'='*60}")
        print("BASELINE: Short Prompt Latency")
        print(f"{'='*60}")
        print(f"Model: {model}")
    ...
    async def test_baseline_latency_medium_prompt(self, model):
        ...
        print(f"\n{'='*60}")
        print("BASELINE: Medium Prompt Latency")
        print(f"{'='*60}")
        print(f"Total time: {result.total_time_sec:.3f}s")
        print(f"TTFT: {result.ttft_sec:.3f}s")
        print(f"Tokens/sec: {result.tokens_per_sec:.1f}")
    ...
    async def test_concurrent_requests(self, model):
        ...
        # Analyze scalability
        print(f"\n{'='*60}")
        print("CONCURRENCY SCALABILITY ANALYSIS")
        print(f"{'='*60}")
    ...
    async def test_long_context_performance(self, model):
        ...
        print(f"\n{'='*60}")
        print("LONG CONTEXT PERFORMANCE")
        print(f"{'='*60}")
